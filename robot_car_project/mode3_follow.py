...
mode3_follow.py
---------------
MODE 3: Bám vật thể bằng cảm biến siêu âm:
...
class FollowObjectFrame(tk.Frame):
    ...
    def _build_follow_controls(self, parent):
        ...
        tk.Label(
            parent,
            text="STM32 tự quét servo và đo HC-SR04",
            bg=config.COLORS["bg"],
        ...
        tk.Label(
            parent,
            text="Đặt vật thể trước xe, sau đó bấm bắt đầu.\n"
            "Camera chỉ dùng để quan sát, không điều khiển xe.",
            bg=config.COLORS["bg"],
            fg="#999999",
            font=("Segoe UI", 8),
            justify="left",
        ...
        self._build_tuning_panel(parent)

    def _build_tuning_panel(self, parent):
        """
        Ô hiệu chỉnh Mode 3 - gửi xuống STM32 bằng lệnh "C3 ..." NGAY
        TRƯỚC lệnh "O" mỗi lần bấm bắt đầu bám (xem toggle_follow()).
        Không cần build lại firmware để đổi các thông số này.
        """
        tk.Label(
            parent,
            text="HIỆU CHỈNH (gửi khi bấm Bắt đầu)",
            bg=config.COLORS["bg"],
            fg=config.COLORS["accent"],
            font=("Segoe UI", 10, "bold"),
        ).pack(anchor="w")

        grid = tk.Frame(parent, bg=config.COLORS["bg"])
        grid.pack(fill="x", pady=(2, 0))

        def make_row(r, label, default_value):
            tk.Label(
                grid, text=label, bg=config.COLORS["bg"], fg=config.COLORS["text"],
                font=("Segoe UI", 9), anchor="w",
            ).grid(row=r, column=0, sticky="w", pady=1)
            entry = ttk.Entry(grid, width=6)
            entry.insert(0, str(default_value))
            entry.grid(row=r, column=1, sticky="e", padx=(4, 0), pady=1)
            return entry

        self.tune_target_entry = make_row(0, "Khoảng cách giữ (cm):", config.MODE3_TARGET_DISTANCE_CM)
        self.tune_tolerance_entry = make_row(1, "Dung sai (cm):", config.MODE3_DISTANCE_TOLERANCE_CM)
        self.tune_max_entry = make_row(2, "Tầm tìm tối đa (cm):", config.MODE3_MAX_FOLLOW_DISTANCE_CM)
        self.tune_move_entry = make_row(3, "PWM tiến/lùi:", config.MODE3_MOVE_PWM)
        self.tune_turn_entry = make_row(4, "PWM quay:", config.MODE3_TURN_PWM)
        grid.columnconfigure(1, weight=1)

        tk.Label(
            parent,
            text="STM32 tự giới hạn về ngưỡng an toàn nếu nhập quá đà.",
            bg=config.COLORS["bg"],
            fg="#999999",
            font=("Segoe UI", 8),
        ).pack(anchor="w", pady=(2, 0))
    ...
    def _read_tuning_values(self):
        """Đọc 5 ô hiệu chỉnh; ô nào trống/sai định dạng thì dùng mặc định trong config."""
        def read_int(entry, default_value):
            try:
        ...
        return (
            read_int(self.tune_target_entry, config.MODE3_TARGET_DISTANCE_CM),
            read_int(self.tune_tolerance_entry, config.MODE3_DISTANCE_TOLERANCE_CM),
            read_int(self.tune_max_entry, config.MODE3_MAX_FOLLOW_DISTANCE_CM),
            read_int(self.tune_move_entry, config.MODE3_MOVE_PWM),
            read_int(self.tune_turn_entry, config.MODE3_TURN_PWM),
        )
    ...
    def toggle_follow(self):
        if self.follow_running:
            ...
            self.follow_info.config(
                text="Trạng thái: quét siêu âm...", fg=config.COLORS["ok"]
            )

            target_cm, tolerance_cm, max_cm, move_pwm, turn_pwm = self._read_tuning_values()
            communication.link.set_follow_tuning(
                target_cm, tolerance_cm, max_cm, move_pwm, turn_pwm
            )
            communication.link.start_follow()
            self._log(
                f"[LỆNH] C3 {target_cm} {tolerance_cm} {max_cm} {move_pwm} {turn_pwm} "
                f"→ START FOLLOW - STM32 tự quét servo + HC-SR04."
            )
            ...
            for kind, payload in communication.link.poll_events():
                if kind == "telemetry":
                    ...
                    if payload == "disconnected" and self.follow_running:
                        ...
                        self.follow_btn.config(text="▶ BẮT ĐẦU BÁM")
                        self.follow_info.config(
                            text="Trạng thái: mất kết nối", fg=config.COLORS["error"]
                        )
                else:
