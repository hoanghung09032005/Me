...
Giao thức:
    ...
    - Mode 3 Tuning : "C3 <target_cm> <tol_cm> <max_cm> <move_pwm> <turn_pwm>\\n"
                      (gửi NGAY TRƯỚC "O" mỗi lần bắt đầu bám - xem set_follow_tuning())
    - Mode 4 AI Line: "I <error_x100>\\n"

[SUA] "O <x_err> <y_err>" (AI Camera cho Mode 3) đã bị VÔ HIỆU HÓA phía
STM32 - Mode3_Follow_SetError() giờ là stub rỗng trong mode3_follow.c.
send_follow_error() bên dưới được giữ lại CHỈ để không phá API, gọi vào
không còn tác dụng điều khiển xe.
"""
...
class CarLink:
    ...
    def set_follow_tuning(self, target_cm, tolerance_cm, max_cm, move_pwm, turn_pwm):
        """
        Gửi thông số hiệu chỉnh Mode 3 xuống STM32 - PHẢI gọi TRƯỚC
        start_follow() mỗi lần bắt đầu bám, vì main.c chỉ gọi
        Mode3_Follow_SetTuning() khi nhận đúng lệnh "C3 ..." này.
        STM32 tự clamp về giới hạn an toàn nếu giá trị gửi lên vượt
        ngưỡng (xem Mode3_Follow_SetTuning trong mode3_follow.c), nên
        không cần validate lại ở đây.
        """
        return self.send_raw(
            f"C3 {int(target_cm)} {int(tolerance_cm)} {int(max_cm)} "
            f"{int(move_pwm)} {int(turn_pwm)}"
        )
    ...
    def start_ai_line(self):
        ...
        FIX BUG 1: TRƯỚC ĐÂY gửi "I 0" - nhưng error_x100=0 là giá trị HỢP LỆ
        (line đúng giữa tâm), đúng kiểu sentinel-collision đã từng gặp và tự
        fix cho AI_LINE_LOST_SENTINEL. Hệ quả: line_detected=1 bị set NGAY
        lúc bấm nút, trước khi có frame thật nào - xe chạy full base speed
        trong lúc chờ frame đầu tiên tới.
        Giờ gửi đúng sentinel "mất line" (khớp AI_NO_LINE_SENTINEL bên STM32,
        xem Mode4_AI_SetError()) - line_detected chỉ được set khi nhận error
        thật đầu tiên từ _process_frame()."""
        return self.send_raw(f"I {config.AI_LINE_LOST_SENTINEL}")
    ...
    def set_obstacle_avoid(self, enabled):
        """
        Bật/tắt hành vi NÉ VẬT CẢN (dừng - quét servo - rẽ tránh) khi xe
        đang chạy PID bám line 5 mắt IR ở Mode 2/Auto, KHÔNG ảnh hưởng gì
        tới bản thân việc bám line. Thêm riêng cho Mode 4 (mode4_ai.py):
        nhánh 'MODE2_PID' của Mode 4 gọi start_auto() để tái dùng PID bám
        line của Mode 2, nhưng Mode 4 tự xử lý vật cản/ghi nhãn qua camera
        ở phía PC nên KHÔNG muốn xe tự dừng-rẽ như Mode 2 gốc - nếu để
        vậy, xe sẽ tự ý rẽ giữa lúc PC đang ghi mẫu train, làm nhiễu cả
        hành trình lẫn dataset.

        THỨ TỰ QUAN TRỌNG: phải gọi hàm này SAU start_auto() ("A"), không
        phải trước - vì Mode2_Obstacle_Init() bên STM32 (chạy trong lúc xử
        lý lệnh "A") luôn reset cờ né vật cản về "bật" mỗi lần vào lại
        Auto. Gọi trước sẽ bị "A" ghi đè lại thành bật ngay sau đó.

        Mode 2 gốc (mode2_auto.py) KHÔNG cần gọi hàm này - mặc định bên
        STM32 đã là bật, giữ nguyên hành vi né vật cản như cũ.

        CẦN firmware STM32 đã có case 'N' xử lý lệnh này trong main.c
        (Mode2_Obstacle_SetAvoidEnabled). Nếu main.c chưa được cập nhật,
        lệnh "N ..." sẽ rơi vào default (bị bỏ qua im lặng) và né vật cản
        vẫn giữ nguyên mặc định là bật - không lỗi, nhưng cũng không tắt
        được cho tới khi nạp lại firmware.
        """
        return self.send_raw(f"N {1 if enabled else 0}")
