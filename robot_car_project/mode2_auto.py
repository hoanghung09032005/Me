    ...
    def _update_telemetry(self, data):
        ...
        if dist > 0 and dist < 20:
            self.dist_label.config(
                text=f"Vật cản: {dist} cm (ĐANG NÉ!)", fg=config.COLORS["error"]
            )
        else:
            self.dist_label.config(text=f"Khoảng cách: {dist} cm", fg="#ffcc00")

    def _on_speed_change(self, value):
        self.speed_display_label.config(text=f"Tốc độ: {int(float(value))}%")
