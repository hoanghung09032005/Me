...
def train():
    ...
    # LƯU Ý QUAN TRỌNG (dễ tưởng nhầm là treo máy): lần GỌI ĐẦU TIÊN tới
    # torch.optim.* trong 1 tiến trình Python, PyTorch bản mới sẽ tự import
    # ngầm torch._dynamo -> kéo theo cả thư viện sympy (rất nặng). Trên
    # Windows, lần import đầu này có thể mất 1-3 phút (lâu hơn nếu Windows
    # Defender đang quét real-time từng file .py của sympy) - KHÔNG PHẢI
    # LỖI, đừng Ctrl+C ở bước này dù terminal có vẻ đứng im.
    print("Đang khởi tạo optimizer (lần đầu có thể mất 1-3 phút do PyTorch "
          "phải import ngầm torch._dynamo/sympy - ĐỪNG Ctrl+C, cứ để nó chạy)...")
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)
    print("Đã khởi tạo optimizer xong, bắt đầu train...")
