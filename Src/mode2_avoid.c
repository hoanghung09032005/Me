...
/* Bật theo mặc định (an toàn cho Mode 2 gốc). Mode 4 (mode4_ai.py, qua
 * Mode2_Obstacle_SetAvoidEnabled() trong mode2_obstacle.c) tắt cờ này sau
 * khi vào Auto để tái dùng PID bám line mà không kèm hành vi né vật cản.
 * Avoid_Init() LUÔN reset cờ về bật mỗi lần vào lại Auto - xem giải thích
 * đầy đủ trong mode2_avoid.h. */
static volatile uint8_t avoid_enabled = 1;
...
{
    avoid_enabled = enabled ? 1U : 0U;

    if (!avoid_enabled && avoid_state != AVOID_NONE) {
        /* Đang giữa chừng 1 pha né (lách/quặt/nắn) mà bị tắt giữa chừng -
         * huỷ ngay, trả servo về giữa, dừng động cơ, để LinePID_Run() tiếp
         * quản ngay ở tick kế tiếp thay vì để xe kẹt giữa pha lách. */
...
int Avoid_Update(uint8_t raw_state, uint8_t side_left, uint8_t side_right)
{
     ...
     * LƯU Ý: việc đo khoảng cách/ping ở trên và bên dưới KHÔNG bị
     * avoid_enabled chặn - Mode 4 vẫn cần field "dist" trong telemetry dù
     * đã tắt hành vi né; chỉ riêng dòng kiểm tra ngưỡng để CHUYỂN sang
     * AVOID_SCAN mới bị chặn bởi avoid_enabled. */
