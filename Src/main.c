...
static void Process_Command(const char *command)
{
    ...
    switch (command_type) {
        ...
        case 'O': {
            ...
            /* FIX BUG 1: Chỉ full-init khi mới vào MODE_FOLLOW lần đầu.
             * Nếu đã ở MODE_FOLLOW, chỉ cập nhật error — tránh re-init
             * làm servo giật về giữa và motor bị zero-pulse mỗi frame. */
        ...
        case 'C': {
            /* Hieu chinh Mode 3 tu GUI - GUI gui lenh nay NGAY TRUOC 'O'
             * moi lan bat dau bam, khong can build lai firmware moi lan
             * doi thong so tren xe that:
             * C3 <target_cm> <tol_cm> <max_cm> <move_pwm> <turn_pwm>
             * Cac gia tri se duoc Mode3_Follow_SetTuning() tu clamp ve
             * gioi han an toan, khong can kiem tra o day. */
            int target_cm, tolerance_cm, max_cm, move_pwm, turn_pwm;
            if (sscanf(command, "C3 %d %d %d %d %d",
                       &target_cm, &tolerance_cm, &max_cm,
                       &move_pwm, &turn_pwm) == 5) {
                Mode3_Follow_SetTuning(target_cm, tolerance_cm, max_cm,
                                       move_pwm, turn_pwm);
                UART_SendString("ACK,C3\n");
            } else {
                UART_SendString("ERR,C3,FORMAT\n");
            }
            break;
        }
        ...
        case 'N': {
            /* Bật/tắt hành vi né vật cản (dừng-quét servo-lách) của Mode 2,
             * KHÔNG đụng gì tới PID bám line. Dùng riêng cho Mode 4
             * (mode4_ai.py qua communication.link.set_obstacle_avoid()):
             * Mode 4 tái dùng PID bám line của Mode 2 (gửi "A" như bình
             * thường) nhưng tự xử lý vật cản/ghi nhãn bằng camera ở phía
             * PC, nên gửi thêm "N0" SAU "A" để tắt hành vi né vật lý.
             * LƯU Ý THỨ TỰ: phải gửi SAU "A" - case 'A' ở trên gọi
             * Mode2_Obstacle_Init(), mà Init() luôn reset cờ này về bật. */
            long enabled = strtol(command + 1, NULL, 10);
            Mode2_Obstacle_SetAvoidEnabled(enabled != 0);
            UART_SendString("ACK,N\n");
            break;
        }
