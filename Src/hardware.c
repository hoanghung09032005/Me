...
/* [THEM] Dem so lan HCSR04_Service() thuc su hoan tat 1 phep do (khong
 * dem luc chi dat co yeu cau). Ben goi (mode3_follow.c) luu lai gia tri
 * nay NGAY LUC goi HCSR04_RequestMeasurement(), sau do so sanh voi gia tri
 * hien tai de biet CHINH XAC ket qua doc duoc co phai cua phep do vua yeu
 * cau hay khong - thay vi doan mo bang cach doi du so tick roi doc bien
 * dung chung (co the trung echo cu neu HCSR04_Service() vi ly do nao do
 * chay cham hon 1 tick so voi du kien). */
static volatile uint32_t hcsr04_measurement_sequence = 0;
...
static uint8_t DHT22_Read(int16_t *temperature_c_x10, int16_t *humidity_rh_x10)
{
     ...
     *
     * LƯU Ý (từ khi thêm EXTI_SideSensors_Init): trong đúng khoảng
     * __disable_irq() này, EXTI trên PB13/PB14 cũng bị chặn giống TIM3 -
     * một xung cực ngắn trên mắt biên rơi đúng vào ~5ms này vẫn có thể bị
     * bỏ lỡ. Đây là điểm mù rất nhỏ và hiếm (5ms mỗi 2 giây), không phải
     * nguyên nhân chính của vấn đề góc cua đã quan sát, nhưng ghi chú lại
     * để không nhầm lẫn nếu sau này gỡ lỗi thêm. */
    __disable_irq();
...
void HCSR04_Service(void)
{
    ...
    /* Tang sequence SAU CUNG, khi hcsr04_distance_cm_x10 da chac chan la
     * ket qua cua phep do nay - ben goi doc thay sequence doi la biet gia
     * tri khoang cach da san sang, khong can doan so tick. */
    hcsr04_measurement_sequence++;
}
...
uint32_t HCSR04_GetMeasurementSequence(void)
{
    return hcsr04_measurement_sequence;
}

uint8_t HCSR04_IsMeasurementPending(void)
{
    return hcsr04_measurement_requested;
}
...
/* ======================================================================
 * EXTI - ngắt ngoài cho 2 mắt biên (PB13/PB14)
 * ----------------------------------------------------------------------
 * Chỉ lo phần CẤU HÌNH NGOẠI VI thuần tuý (AFIO chọn port, RTSR/FTSR chọn
 * cạnh, NVIC bật ngắt). KHÔNG đọc GPIO/polarity ở đây - việc đó thuộc về
 * main.c (đã có sẵn macro SIDE_SEES_LINE áp dụng LINE_SIDE_ACTIVE_LOW),
 * để mọi quyết định về "mức nào nghĩa là gặp vạch" chỉ tập trung ở 1 chỗ
 * duy nhất như main.c đã ghi chú.
 * ====================================================================== */
void EXTI_SideSensors_Init(void)
{
    RCC->APB2ENR |= RCC_APB2ENR_AFIOEN;

    /* EXTI13/14 map sang GPIOB qua AFIO_EXTICR4 (EXTICR[3] theo CMSIS,
     * 0-indexed): EXTI12 ở bit[3:0], EXTI13 ở bit[7:4], EXTI14 ở
     * bit[11:8], EXTI15 ở bit[15:12]. Giá trị 0x1 = chọn port B cho line
     * đó. Chỉ đụng đúng 2 nhóm bit của EXTI13/EXTI14, không đụng EXTI12/15
     * (không dùng ở đây, tránh phá cấu hình khác nếu có sau này). */
    AFIO->EXTICR[3] = (AFIO->EXTICR[3] & ~((0xFU << 4) | (0xFU << 8))) |
                        ((0x1U << 4) | (0x1U << 8));

    /* Bắt CẢ HAI cạnh lên/xuống - cần biết MỌI lần đổi mức trên 2 chân
     * này, vì hướng "gặp vạch hay không" phụ thuộc LINE_SIDE_ACTIVE_LOW
     * (có thể là cạnh lên hoặc cạnh xuống tuỳ polarity thực tế). */
    EXTI->RTSR |= (1U << 13) | (1U << 14);
    EXTI->FTSR |= (1U << 13) | (1U << 14);

    EXTI->PR = (1U << 13) | (1U << 14);   /* xoá cờ pending cũ (nếu có) trước khi bật */
    EXTI->IMR |= (1U << 13) | (1U << 14);

    NVIC_ClearPendingIRQ(EXTI15_10_IRQn);
    /* ISR này chỉ ghi vài biến volatile (vài chục ns), vô hại với TIM3 dù
     * cùng mức ưu tiên, nhưng vẫn đặt THẤP HƠN TIM3 (số lớn hơn 0) - đúng
     * nguyên tắc đã áp dụng cho USART1 ở trên: KHÔNG có ngoại lệ nào được
     * phép trễ vòng điều khiển động cơ 10ms. */
