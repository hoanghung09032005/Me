 ...
 * File này chỉ còn 4 việc: khởi tạo cả 2 module, xuất PWM (bù trừ +
 * telemetry, dùng CHUNG cho cả 2 module qua mode2_internal.h), mỗi
 * tick quyết định gọi module nào, và expose 1 công tắc bật/tắt né vật
 * cản (Mode2_Obstacle_SetAvoidEnabled) cho main.c - chỉ chuyển tiếp
 * xuống Avoid_SetEnabled() trong mode2_avoid.c, không tự chứa logic gì.
 */
...
{
    /* Chỉ chuyển tiếp - toàn bộ logic huỷ giữa chừng/reset trạng thái nằm
     * trong mode2_avoid.c, đúng chỗ dữ liệu née thuộc về. Thêm cho Mode 4
     * (mode4_ai.py): tái dùng PID bám line của Mode 2 nhưng không muốn xe
     * tự dừng-lách vật cản trong lúc PC đang ghi nhãn train qua camera. */
    Avoid_SetEnabled(enabled);
}
...
void Mode2_Obstacle_Update(uint8_t raw_state, uint8_t side_left, uint8_t side_right)
{
     ...
     * không (đang né, hoặc vừa phát hiện vật cản và bắt đầu né - hoặc
     * hoàn toàn bỏ qua nếu Avoid_SetEnabled(0) đã được gọi) - trả về
